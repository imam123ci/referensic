...
function finishTest(){
    Timer.stop();
    $(".mui-container-fluid .button-borders").toggleClass("hide","show");
    $("#stage").html("");
...
function nextQuestion( prefix ){
  ...
  $("#stage").load(prefix + "/" + questionID + ".htm?QN=" + currentQ, function(responseText, textStatus, jqXHR){
      selected = 0;
     ...
     $(".Q p>span").html("<span>Q " + currentQ + " </span>");
     if (timeout) {
         $("#stage").html("");
...
function previousQuestion( prefix ){
  ...
  $("#stage").load(prefix + "/" + questionID + ".htm?QN=" + currentQ, function(responseText, textStatus, jqXHR){
      ...
      $(".Q span").html("<span>Q " + currentQ + " </span>");
      $(".Q a").click(function(){
         /* Get the index of clicked option */
         var index = $( ".Q a" ).index( this );
         questionMap[currentQ].selected = index;
